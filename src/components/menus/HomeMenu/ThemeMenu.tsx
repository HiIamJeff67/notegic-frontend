    ...
    <HomeMenuSection label={t("themes.theme")}>
      <div className="max-h-[13rem] overflow-y-auto">
        {themeManager.availableThemes.map((theme, index) => (
          <div className="flex flex-col items-start" key={theme.id}>
            <button
              className="flex min-w-0 w-full cursor-pointer items-start gap-3 py-1.5 text-left text-sm font-medium text-foreground transition-colors hover:text-foreground disabled:cursor-wait disabled:opacity-50"
